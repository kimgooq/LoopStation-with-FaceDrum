...
function onResults2(results) {
  ...
  if (results.poseLandmarks) {
    ...
    // let testbones = [];
    // testbones.push(jointLeftShoulder_inside); //이게 아마 기본
    // testbones.push(jointLeftShoulder);
    // testbones.push(jointLeftElbow);
    // testbones.push(jointLeftWrist);
  ...
  console.log(skeleton.bones[33].position);
  skeleton.bones[33].position.x = null;
  skeleton.bones[33].position.y = null;
  skeleton.bones[33].position.z = null;
  skeleton.bones[34].position.x = null;
  skeleton.bones[34].position.y = null;
  skeleton.bones[34].position.z = null;
  ...
  const rightarmiks = [
    {
        target: 36, // "target"
        effector: 36, // "bone3"
        links: [{index: 35},{ index: 34 },{index:33} ] // "bone2", "bone1", "bone0"
    }
];
...
const leftarmiks = [
    {
        target: 12, // "target"
        effector: 12, // "bone3"
        links: [{index: 11},{ index: 10 },{index: 9} ] // "bone2", "bone1", "bone0"
    }
];
...
let ikSolverra = new CCDIKSolver( skeleton, rightarmiks );
let ikSolverla = new CCDIKSolver(skeleton,leftarmiks);
let ikSolverrl = new CCDIKSolver(skeleton,rightlegiks);
let iksolverll = new CCDIKSolver(skeleton,leftlegiks);
function render() {
  // console.log("업데이트 확인");
    ikSolverla?.update();
    ikSolverra?.update();
