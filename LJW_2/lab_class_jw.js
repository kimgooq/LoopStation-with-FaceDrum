...
function onResults2(results) {
  ...
  function SetRbyCalculatingJoints(
    ...
    const v = new THREE.Vector3()
      ...
      .normalize();
    if (joint_model_parent == null) {
      R = computeR(joint_model_child.position.clone().normalize(), v);
      // console.log("SetRbyCalculatingJoints() joint_model_parent is null");
...

  {
    ...
    // left upleg-leg-foot
    let R_chain_leftupper = new THREE.Matrix4().identity();
    ...
    let R_chain_rightupper = new THREE.Matrix4().identity();
    ...
    let R_chain_body_spine = new THREE.Matrix4().identity();
    const jointNeck = newJoints3D["neck"];
    const jointHip = newJoints3D["hips"];
    const jointSpine0 = newJoints3D["spine0"];
    const jointSpine1 = newJoints3D["spine1"];
    const jointSpine2 = newJoints3D["spine2"];

    const boneSpine0 = skeleton.getBoneByName("mixamorigSpine");
    const boneSpine1 = skeleton.getBoneByName("mixamorigSpine1");
    const boneSpine2 = skeleton.getBoneByName("mixamorigSpine2");
