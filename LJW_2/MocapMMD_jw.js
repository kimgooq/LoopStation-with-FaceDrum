...
// import { CCDIKSolver } from "https://unpkg.com/three@0.141.0/examples/jsm/animation/CCDIKSolver.js";
...
function init() {
  ...
  // camera_ar.position.set(1, 15, 30);
  // camera_ar.up.set(0, 0, 0);
  ...
  const ground_mesh = new THREE.Mesh(new THREE.PlaneGeometry(1000, 1000),
  ...
  loader.load("./PMD/pmd_model.pmd", function (object) {
    ...
    model.traverse(function (object) {
      if (object.isBone) {
        ...
        bones.push(object);
        // bookmark
        // console.log(object.name); (이거는 이름 알려주는거)
        // console.log(object.position); (이거는 벡터값 알려주는거))

        // object.position.multiplyScalar(0.1);
        // console.log(object); //차일드 알려주는거 같음, 이게 더 중요한 거 같은데...?

        model.geometry.bones[i].name = EnglishBoneName[i];
        // model.geometry.bones[i].scl = [0.1, 0.1, 0.1];
        // console.log(model.geometry.bones);
        // console.log(object);
        // object.scale.set(0.1, 0.1, 0.1);

        // physicsHelper.physics.bodies[i].bone.scale.set(0.1, 0.1, 0.1);
        i++;
      }
      if (object.isPhysics) {
        console.log(object);
    ...
    console.log("확인용");
    console.log(bones);
    // model.geometry.bones.forEach(function (obj) {
    //   obj.pos = [obj.pos[0] * 0.1, obj.pos[1] * 0.1, obj.pos[2] * 0.1];
    //   //   console.log(obj);
    // });
    ...
    // model.add( bones[ 0 ] ); // "root" bone
    // physics = new THREE.MMDPhysics(model);
    // model.bind( skeleton );  
  ...
  function onResults2(results) {
    ...
    if (results.poseLandmarks) {
      ...
      //   console.log(skeleton.getBoneByName("Left shoulder P").position);
      //   console.log(skeleton.getBoneByName("left shoulder").position);
      //   console.log(skeleton.getBoneByName("Left shoulder C").position);
      //   skeleton
      //     .getBoneByName("Left shoulder P")
      //     .quaternion.setFromRotationMatrix(test_R);
      ...
      // const jointLeftHeel = pos_3d_landmarks["left_heel"];
      ...
      // const jointRightHeel = pos_3d_landmarks["right_heel"];
      ...
      //bookmark3
      const backhair1 = skeleton.getBoneByName("Back hair 1"); //67
      const backhair2 = skeleton.getBoneByName("Back hair 2"); //68
      const backhair3 = skeleton.getBoneByName("Back hair 3"); //69
      const backhair4 = skeleton.getBoneByName("Back hair 4"); //70
      const righthair1 = skeleton.getBoneByName("Right side hair 1"); //62
      const righthair2 = skeleton.getBoneByName("Right side hair 2"); //63
      const righthair3 = skeleton.getBoneByName("Right side hair 3"); //64
      const righthair4 = skeleton.getBoneByName("Right side hair 4"); //65
      const righthair5 = skeleton.getBoneByName("Right side hair 5"); //66
      const lefthair1 = skeleton.getBoneByName("Left side hair 1"); //57
      const lefthair2 = skeleton.getBoneByName("Left side hair 2"); //58
      const lefthair3 = skeleton.getBoneByName("Left side hair 3"); //59
      const lefthair4 = skeleton.getBoneByName("Left side hair 4"); //60
      const lefthair5 = skeleton.getBoneByName("Left side hair 5"); //61
    ...
    const righthair = [
      {
          target: 142, // "target"
          effector: 61, // "bone3"
          links: [{index:60},{ index: 59 },{index: 58},{index: 57} ] // "bone2", "bone1", "bone0"
          //141이 허리 중앙점임
          //142가 중앙종착지
      }
    ];
    const lefthair = [
      {
          target: 142, // "target"
          effector: 66, // "bone3"
          links: [{index:65},{ index: 64 },{index: 63},{index: 62} ] // "bone2", "bone1", "bone0"
          //141이 허리 중앙점임
          //142가 중앙종착지
      }
    ];

    let ikSolverhair = new CCDIKSolver( skeleton, backhair );
    let ikSolverrhair = new CCDIKSolver( skeleton, righthair );
    let ikSolverlhair = new CCDIKSolver( skeleton, lefthair );
    function render() {
      ikSolverrhair?.update();
      ikSolverhair?.update();
      ikSolverlhair?.update();
    ...
    render();
      ...
      //   skeleton.getBoneByName("right toe").clone().getWorldPosition(target);
      //   console.log("local", skeleton.getBoneByName("right toe").position);
      ...
      //   const v_index = new THREE.Vector3().subVectors(
      //     jointIndex_mcp,
      //     jointWrist
      //   );
      ...
      //   const v_pinky = new THREE.Vector3().subVectors(
      //     jointPinky_mcp,
      //     jointWrist
      //   );
      //   const v_thumb = new THREE.Vector3().subVectors(
      //     jointThumb_cmc,
      //     jointWrist
      //   );
