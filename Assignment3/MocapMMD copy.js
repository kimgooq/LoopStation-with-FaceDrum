...
function init() {
  ...
  let model,
    ...
    skeleton_helper,
    physics_model,
    physics_skeleton = null,
    physics_skeleton_helper;
  ...
  loader.load("./PMD/pmd_model.pmd", function (object) {
    ...
    // physics_model.geometry.userData.MMD.rigidBodies.forEach(function (obj) {
    //   obj.position = [
    //     obj.position[0] * 0.1,
    //     obj.position[1] * 0.1,
    //     obj.position[2] * 0.1,
    ...
    //   obj.weight *= 0.1;
    // });
    // physics_model.geometry.userData.MMD.constraints.forEach(function (obj) {
    //   // obj.position = [
    //   //   obj.position[0] * 0.1,
    //   //   obj.position[1] * 0.1,
    //   //   obj.position[2] * 0.1,
  ...
  loader.load("./PMD/pmd_model.pmd", function (object) {
    ...
    scene.add(physics_model);
    let physics_bone = [];
    i = 0;
    physics_model.traverse(function (object) {
      if (object.isBone) {
        object.name = EnglishBoneName[i];
        physics_bone.push(object);
        object.position.multiplyScalar(0.1);
        i++;
      }
    });
    ...
    helper.add(physics_model, {
      physics: true,
    });
    physicsHelper = helper.objects.get(physics_model).physics.createHelper();
    physicsHelper.visible = false;
    scene.add(physicsHelper);
    physics_skeleton = new THREE.Skeleton(physics_bone);
    physics_skeleton_helper = new THREE.SkeletonHelper(model);
    physics_skeleton_helper.visible = true;
    scene.add(physics_skeleton_helper);
  });
  ...
  function onResults2(results) {
    ...
    function SetRbyCalculatingJoints(
      ...
      if (physics_joint != false)
        physics_joint.quaternion.setFromRotationMatrix(R);
    ...
    if (results.poseLandmarks) {
      ...
      SetRbyCalculatingJoints(
        ...
        boneHead,
        R_chain_neck,
        physics_skeleton.getBoneByName("neck")
      );
      ...
      physics_skeleton.getBoneByName("head").quaternion.copy(Q_Head);
      ...
      SetRbyCalculatingJoints(
        ...
        boneLeftArm,
        R_chain_leftupper,
        physics_skeleton.getBoneByName("Left shoulder P")
      );
      SetRbyCalculatingJoints(
        jointLeftShoulder,
        jointLeftElbow,
        ...
        boneLeftForeArm,
        R_chain_leftupper,
        physics_skeleton.getBoneByName("Left shoulder C")
      );
      SetRbyCalculatingJoints(
        jointLeftElbow,
        jointLeftWrist,
        ...
        boneLeftHand,
        R_chain_leftupper,
        physics_skeleton.getBoneByName("left elbow")
      );
