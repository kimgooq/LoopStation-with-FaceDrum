...
import { FBXLoader } from "https://unpkg.com/three@0.133.1/examples/jsm/loaders/FBXLoader.js";
// import { AmmoPhysics } from "https://unpkg.com/three@0.133.1/examples/jsm/physics/AmmoPhysics.js";
import { AmmoPhysics } from "./custom_AmmoPhysics.js";
...
async function init() {
  ...
  ground_mesh.position.set(0, -0.5, 0);
  ...
  loader.load("./jerry.fbx", function (object) {
    model = object;
    console.log(object);
    object.scale.set(0.01, 0.01, 0.01);
    scene.add(model);
    // physics.addMesh(model);
    let bones = [];
    ...
    console.log(model_mesh);
    physics.addMesh(model_mesh, 1);
  ...
  function onResults2(results) {
    ...
    if (results.poseLandmarks) {
      ...
      // const boneRightShoulder = skeleton.getBoneByName(
    ...
    if (results.leftHandLandmarks) {
      ...
      const v_hand_vertical = v_middle
        .clone()
        .applyMatrix4(R_chain_leftupper.clone().transpose());

      const boneVertical = boneIndex1.position.clone();
      ...
      const v_hand_horizontal = new THREE.Vector3()
        .subVectors(jointRing_mcp, jointIndex_mcp)
        .applyMatrix4(R_chain_leftupper.clone().transpose());
      ...
      boneHand.quaternion.setFromRotationMatrix(R_hand_y);
      ...
      for (i = 0; i < 12; i++) {
        ...
        let Bone =
          "mixamorigLeftHand" + bone_list[quotient + 4] + (remainder + 1);
        let next_Bone =
          "mixamorigLeftHand" + bone_list[quotient + 4] + (remainder + 2);
    ...
    if (results.rightHandLandmarks) {
      ...
      //   let next_point = finger + "_finger_" + bone_point_list[remainder + 1];
      //   let Bone =
      //     "mixamorigRightHand" + bone_list[quotient + 4] + (remainder + 1);
      //   let next_Bone =
      //     "mixamorigRightHand" + bone_list[quotient + 4] + (remainder + 2);
      //   let R = R_list[quotient];
      //   SetRbyCalculatingJoints(
      //     hand_3d_landmarks[finger_point],
      //     hand_3d_landmarks[next_point],
    ...
    // position.set(0, Math.random() + 1, 0.2);
